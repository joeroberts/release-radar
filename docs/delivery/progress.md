...
**Current blocker:** Main launched the installed app (PID 6000), then the supported evidence query returned `appUnavailable`. At 04:43:06 EDT, bridge PID 6016 rejected connector PID 87080 with code-signing requirement failure. Read-only `lsof` confirms PID 87080 still maps `/Applications/.ReleaseRadar.backup.5088.31090/Contents/Helpers/ReleaseRadarAgentTools`. This session does not expose the new `release_radar_documentation_catalog_transition` tool. Owner must restart Codex to reload its connector/tool inventory; then Main will run the supported read-only diagnostic. No direct helper fallback, catalog mutation or acceptance retry occurred. This recurrence belongs to the still-open Outcome 3 connector-upgrade correction, not a claim that it is solved by diagnostic release.
