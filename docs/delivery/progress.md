...
- Current remediation state: **Ready for owner validation; not accepted and not Done.** The populated-schema SQLite-23 repair, independent engineering reviews, nonlaunch Release stage/install, and actual-artifact QA/Security gates are complete with zero open Required findings. The exact verified Release bundle is preserved under `dist/` and installed at `/Applications/ReleaseRadar.app`. Only the owner's personal runtime test and explicit approval may change this state to Done or Accepted.
- Next eligible remediation work: none. The owner personally launches `/Applications/ReleaseRadar.app`, repeats **Initialize Project Tracking** against the intended populated workflow, verifies the SQLite-23 failure is absent and the pending tracking state persists, then explicitly approves or reports the observed failure. No later product writer is released automatically; Help, Portable Import/exporter work, and unrelated warning cleanup remain closed.
...
- Nonblocking next-phase candidates: productionize the owner-approved wordmark recorded in `docs/brand/README.md`; remove the Swift optional-`.none` and test actor-isolation warnings; attach live Codex state only when a supported authenticated endpoint exists; and add Developer ID/notarized packaging only if distribution expands beyond the owner Mac. The structure-less onboarding decision is resolved and implemented; only owner validation of the repaired installed workflow remains open.
...
| --- | ---: | --- |
| Active | 1 | Owner runtime validation of the installed SQLite-23 repair. |
| Completed | 20 | RR-01–RR-10 (10), RR-R1–RR-R6 (6), Attach Folder (1), Tasks 7A/7B (2), and absorbed RR-R7 route/sidebar work (1). |
...
| --- | ---: | --- | --- | --- | --- | --- | --- |
| A1 | 1 | SQLite-23 repair owner validation | Active | This ledger, **Current gate** and **Initialize Project Tracking SQLite-23 repair — reopened owner gate** | Engineering, Release staging/install, and artifact reviews are complete. Only the owner's installed-app runtime test and explicit decision survive. | Sole current action; no product writer is eligible. | None; validation uses the installed app. |
...
| S1 | 1 | “Ask agent to define first phase” / request-marker onboarding | Superseded | `docs/design/agent-driven-delivery-dashboard-design.md`, **Onboarding**; this ledger, Task 7B acceptance | Replaced by **Initialize Project Tracking** and a path-free copied-not-sent prompt. | `docs/design/mockups/onboarding_state.png` retains obsolete copy and is historical visual evidence only. | No replacement now: the delivered UI is completed and the current active gate is runtime validation, not design. |
...
| --- | --- | --- | --- | --- | --- |
| P0 / 1 — Owner runtime validation | **Active; sole eligible action** | Owner tests the installed Release workflow and explicitly approves or reports failure. | Installed app and local owner state; no concurrent product writer. | No mockup required. | It is the only current gate and determines whether repair work reopens. |
| P1 / 2 — Coupled product/IA reconciliation | **Closed decision gate** | P0 completion; owner decisions for Phase Board/Project Plan/Work Board, Activity/History, one-to-one/one-to-many links, semantic suggestions, and complete history semantics; ADR update where data authority changes. | Project navigation, board/plan routes, goal-link storage, observation provenance, archive coverage, focus/history state. Serialize as one design stream. | Existing proposal images retained but unapproved; new images only after decisions. | These choices overlap and cannot safely be planned or mocked independently. |
| P2 / 3 — Deterministic wordmark/lockups | **Deferred; separable candidate** | P0 and explicit owner release; production typeface/license or drawn outlines. | Brand files only; may proceed independently from later product IA when released. | Existing raster direction adequate; no new AI mockup. | It is the only surviving visual-production candidate that does not share product routes or data. |
| P3 / 4 — Warning cleanup | **Deferred maintenance** | P0 and explicit release. | Compiler/test files; keep separate from product UI/data work. | None. | Low-risk maintenance, but it must not displace the active gate. |
