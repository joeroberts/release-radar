...
Build Agent integrated the reviewed diagnostic as `e7205e1e` on canonical branch `codex/p6-project-controls`, preserving the earlier reviewed registration repair `78d73e7a`. Release 0.1.28 metadata now carries app/plugin version 0.1.28 (1) and recognized plugin digest `c79d9b7d79bf24c0d11a4e2caf74cf4901b5cd4ae4962dcc7ca160ff8f97cfcd`; five focused version/package tests passed with zero failures. The authorized local package includes only these reviewed repairs; unreviewed Manage Project candidate6ba3 remains excluded. Installer destination is repository `dist/ReleaseRadar-0.1.28.dmg`, with no Downloads copy. Packaging and installation have not yet occurred. Main will notify the owner before local installation and required Codex restart. Push, PR, tag publication, notarization and Release Radar mutation remain unauthorized.
