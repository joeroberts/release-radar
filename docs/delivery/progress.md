...
Main's affected installed UI check reached terminal Remove, then disabled Update displayed the intended project-level Resume guidance and retained the stopped/uncertain-worker warning, with controls enabled. No Save or Resume followed; the disposable workflow remains disabled for inspection. This is Main-observed evidence. The owner authorized and Main applied only two precise `rr-project-ro` user-configuration grants: read `/System/Library/OpenSSL/openssl.cnf` and write `/private/var/folders/g8/tb13s4dd31vbnlx91kq133j40000gn/T/com.openai.sky.CUAService`; TOML and semantic checks passed with no other setting change. The reviewer's fresh runtime confirmed the first grant and subsequently obtained AX/screenshot data; latest commentary confirms the corrected message, callout and controls, but its formal report is `waitingOnApproval`, so independent installed UI PASS remains pending. Normal/narrow onboarding/settings QA, installed mixed-hook feedback and actual second-Mac portability remain open.
...
**September 18 — Outcome 3 completion goal active; historical isolation passed, hook recovery guidance is merged, and formal independent UI reporting remains pending.**
