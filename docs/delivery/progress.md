...
The reviewer’s final bounded UI run passed readable/accessibility/responsive checks for Settings → Connections at normal, expanded and left-half narrow widths (the narrow layout collapses the sidebar), plus normal/narrow initial Add Project chooser with distinct AX Initialize/Attach/Cancel controls. At native zoom in expanded chooser, the title and two choices compressed into the upper-left while almost the entire window remained blank; controls stayed usable and AX-labeled. The reviewer classifies this as a proposed P2 presentation finding, not data loss or a blocked function. Screenshot sizes establish pixels only; native dimensions are unexposed. The window was restored to normal Projects; no folder selection, submission or project mutation occurred. Owner prioritization of this accumulated finding is the next action before any correction or release decision.
